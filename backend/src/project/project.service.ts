...
export class ProjectService {
  ...
  async getMembers(project_id: string, req: Request) {
    ...
    const members_query = this.memberModel.find(filterQuery)
      .select('-__v')
      .skip(skip)
      .limit(limit_number)
      .sort({ createdAt: -1 })
    ...
    const [members, total_docs] = await Promise.all([
      members_query.exec(),
    ...
    return { data: members, page: page_number, limit: limit_number, total_pages, total_count: total_docs, message: 'Members fetched successfully' };
  }
  ...
  async findAll(req: Request) {
    ...
    const projects_query = this.projectModel.find(filterQuery)
    ...
    const [projects, total_docs] = await Promise.all([
      projects_query.exec(),
      total_docs_query.exec()
    ]);
    ...
    return { data: projects, page: page_number, limit: limit_number, total_pages: total_pages, total_count: total_docs, message: 'Projects fetched successfully' };
  }
