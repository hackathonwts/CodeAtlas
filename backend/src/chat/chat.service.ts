...
export class ChatService {
  ...
  async findAll(req: Request) {
    ...
    const chats_query = this.chatModel.find(filterQuery)
      .select('-__v')
      .skip(skip)
      .limit(limit_number)
      .sort({ createdAt: -1 })
    ...
    const [chats, total_docs] = await Promise.all([
      chats_query.exec(),
      total_docs_query.exec()
    ]);

    const total_pages = Math.ceil(total_docs / limit_number);

    return { data: chats, page: page_number, limit: limit_number, total_pages, total_count: total_docs, message: 'Chats fetched successfully' };
  }
  ...
  update(id: string, updateChatDto: UpdateChatDto) {
    return this.chatModel.findByIdAndUpdate(
      id,
      { ...updateChatDto, updatedAt: new Date() },
      { new: true }
    )
      .populate('project_id', 'title')
      .populate('user_id', 'full_name email');
  ...
  async findConversations(chat_id: string, req: Request) {
    ...
    const conversations_query = this.conversationModel.find(filterQuery)
      .select('-__v')
      .skip(skip)
      .limit(limit_number)
      .sort({ createdAt: 1 })
    ...
    const [conversations, total_docs] = await Promise.all([
      conversations_query.exec(),
      total_docs_query.exec()
    ]);

    const total_pages = Math.ceil(total_docs / limit_number);

    return { data: conversations, page: page_number, limit: limit_number, total_pages, total_count: total_docs, message: 'Conversations fetched successfully' };
  }
