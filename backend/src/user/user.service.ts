...
export class UserService {
  ...
  async getMe(user: Partial<IUser>) {
    const user_data = await this.userModel.findById(user._id)
  ...
  async updatePersonalInfo(user: Partial<IUser>, body: UpdatePersonalInfoDto) {
    ...
    const updated_user = await this.userModel.findOneAndUpdate({ _id: user._id }, { $set: updateObject }, { new: true })
      .where({ is_deleted: false })
      .populate({
        path: 'active_role',
        match: { is_deleted: false },
  ...
  async switchUserRole(user: Partial<IUser>, roleId: string) {
    ...
    const updated_user = this.userModel.findById(user._id)
      .where({ is_deleted: false })
      .populate({
        path: 'active_role',
        match: { is_deleted: false },
  ...
  async findAll(req: Request) {
    ...
    const users_query = this.userModel.find(filterQuery)
    ...
    const [users, total_docs] = await Promise.all([
      users_query.exec(),
      total_docs_query
    ]);
    ...
    return { data: users, page: page_number, limit: limit_number, total_pages: total_pages, total_count: total_docs, message: 'Users fetched successfully' };
  }
